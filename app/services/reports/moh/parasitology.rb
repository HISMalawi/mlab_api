...
module Reports
  ...
  module Moh
    ...
    class Parasitology
      ...
      def generate_report
        report_data = insert_into_moh_data_report_table(department: 'Parasitology', time_filter: year,
                                                        action: 'update')
        data = update_report_counts(report_data)
        Reports::Moh::ReportUtils.save_report_to_json('Parasitology', data, year)
        data
      end
