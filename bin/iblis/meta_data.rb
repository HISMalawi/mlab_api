...
ActiveRecord::Base.transaction do
  ...
  specimes.each do | specimen |
    ...
    Specimen.create(name: specimen.name, creator: 1, created_date: specimen.created_at, updated_date: specimen.updated_at)
  end
  # Load test types
  test_types = Iblis.find_by_sql("SELECT tt.name, tt.short_name, tc.name As dept, tt.created_at, tt.updated_at, tt.targetTAT FROM test_types tt INNER JOIN test_categories tc ON tc.id =tt.test_category_id")
  test_types.each do |test_type|
    department = Department.find_by_name(test_type.dept)
    Rails.logger.info("=========Loading test type: #{test_type.name}===========")
    TestType.create(name: test_type.name, department_id: department.id, expected_turn_around_time: test_type.targetTAT, creator: 1, created_date: test_type.created_at, updated_date: test_type.updated_at)
  end
  ...
  testtypes_specimens.each do |tt_sp|
    ...
    SpecimenTestTypeMapping.create(specimen_id: specimen.id, test_type_id: test_type.id, creator: 1, created_date: Time.now, updated_date: Time.now)
  end
  # Load test panels
  ...
  panel_types.each do |panel_type|
    ...
    TestPanel.create(name: panel_type.name, creator: 1,  created_date: panel_type.created_at, updated_date: panel_type.updated_at)
  end
  ...
  test_types_panels.each do |tt_panel|
    ...
    TestTypePanelMapping.create(test_panel_id: test_panel.id, test_type_id: test_type.id, creator: 1, created_date: Time.now, updated_date: Time.now)
  end 
end